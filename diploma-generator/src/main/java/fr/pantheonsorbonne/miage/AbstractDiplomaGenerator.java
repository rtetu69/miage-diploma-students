...
public abstract class AbstractDiplomaGenerator implements DiplomaGenerator {
	 ...
	 */
	@Override
	public InputStream getContent() throws MyRuntimeException {
		...
		try (ByteArrayOutputStream bos = new ByteArrayOutputStream();) {
			...
			throw new MyRuntimeException("failed to generate the file to stream to", e);
		}

	}

	protected void writeToStream(OutputStream os) throws MyRuntimeException {
		...
		try {
			...
			throw new MyRuntimeException("failed to generate Document", e);
		} finally {
			document.close();
		}
	}
